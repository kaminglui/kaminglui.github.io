...
function signSeries(values, deadband = 0.05) {
  const signs = [];
  let last = 0;
  values.forEach((v) => {
    const next = Math.abs(v) <= deadband ? last : Math.sign(v) || last;
    signs.push(next);
    last = next;
  });
  return signs;
...
function countTransitions(values, deadband = 0.05) {
  const signs = signSeries(values, deadband);
  let flips = 0;
  for (let i = 1; i < signs.length; i += 1) {
    if (signs[i] !== 0 && signs[i] !== signs[i - 1]) flips += 1;
  }
  return flips;
}
...
describe('op-amp comparator mode', () => {
  ...
  it('avoids chatter on a slow threshold crossing', () => {
    ...
    const transitions = countTransitions(outputs, 0.05);
  ...
  it('holds state when the inputs sit at the threshold', () => {
    const circuit = buildComparator();
    circuit.vin.props.Vdc = 3;
    runDC(circuit);
    ...
    const { samples } = runTransient(circuit, {
      ...
      dt: 2e-4,
      onStep: ({ t }) => {
        if (t > 0.0008) circuit.vin.props.Vdc = 2.5;
      },
      measure: ({ sim }) => ({ vout: sim.voltage(circuit.op, 0) })
    });
    const outputs = samples.map((s) => s.vout);
    const transitions = countTransitions(outputs, 0.05);
    expect(transitions).toBeLessThanOrEqual(1);
    const signs = signSeries(outputs, 0.05);
    expect(signs[signs.length - 1]).toBe(signs[0]);
  });

  it('flips once per crossing for a slow sine input', () => {
    const circuit = buildComparator();
    const duration = 0.01; // one 100 Hz cycle
    const freq = 100;
    const amplitude = 2.5;
    const offset = 2.5;
    ...
    const { samples } = runTransient(circuit, {
      ...
      onStep: ({ t }) => {
        ...
        circuit.vin.props.Vdc = offset + amplitude * Math.sin(theta);
      },
      measure: ({ sim }) => ({ vout: sim.voltage(circuit.op, 0) })
    });
